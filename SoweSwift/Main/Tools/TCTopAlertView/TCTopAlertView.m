...
#define TCT_Color(r, g, b) TCT_ColorWithAlpha(r, g, b, 1)
#define TCT_ColorWithAlpha(r, g, b, a) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:(a)]
...
+ (TCTopAlertViewSetting *)shareInstance {
    ...
    dispatch_once(&onceBlock, ^{
        ...
        singleton.height = 64;
        singleton.backgroundColor = TCT_Color(0, 0, 0);
...
/**创建一个AlertView*/
+ (TCTopAlertView *)topAlertViewWithMessage:(NSString *)message {
    // 创建一个AlertView实例对象
    TCTopAlertView *topAlertView = [[TCTopAlertView alloc] init];
    // 设置显示消息
...
+ (void)showMessage:(NSString *)message inView:(UIView *)view completion:(void (^)(void))completion {
    ...
    TCTopAlertView *topAlertView = [self topAlertViewWithMessage:message];
